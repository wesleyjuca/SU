  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
            ...
            <tbody>
              {data.usuarios.map((u) => (
                <tr key={u.user_id}>
                  ...
                  <td className="text-right text-sm text-afj-black/50">
                    {data.total_custo_usd > 0 ? `${((u.custo_usd / data.total_custo_usd) * 100).toFixed(1)}%` : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="flex items-start gap-2 text-xs text-afj-black/50 bg-afj-cream/60 border border-afj-cream-dark rounded-sm p-3">
        ...
        <span>
          <strong className="text-afj-black/70">Em evolução:</strong> limites de consumo configuráveis por usuário e
          alertas de orçamento chegarão em uma próxima etapa. Custos de chaves próprias (BYOK) são pagos
          diretamente pelo usuário ao provedor e aparecem aqui apenas como estimativa.
        </span>
      </div>
    </div>
  );
