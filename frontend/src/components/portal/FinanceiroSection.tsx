...
import { DollarSign, CheckCircle, Clock, ChevronDown, ChevronRight } from "lucide-react";
 ...
 * a rota `/portal/financeiro`). Fechada por padrão. */
export function FinanceiroSection() {
  ...
  useEffect(() => {
    ...
    portalApi.get<PortalFinanceiro[]>("/portal/financial")
      .then((data) => { setEntries(data); setCarregado(true); })
      .catch(() => toast.error("Erro ao carregar dados financeiros."))
      .finally(() => setLoading(false));
  }, [aberto, carregado, toast]);
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-semibold text-gray-800">Financeiro</h2>
        </div>
        ...
        <div className="border-t border-gray-100 p-4 space-y-4">
          {loading ? (
            <div className="grid grid-cols-2 gap-4">
              {[1, 2].map((i) => <div key={i} className="bg-gray-100 rounded-lg h-20 animate-pulse" />)}
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-amber-50/60 rounded-lg border border-amber-200 p-4">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    <Clock size={16} className="text-amber-600" />
                    <p className="text-xs font-medium text-gray-600">Valor Pendente</p>
                  </div>
                  <p className="text-xl font-bold text-amber-700">{formatBRL(totalPendente)}</p>
                </div>
                <div className="bg-green-50/60 rounded-lg border border-green-200 p-4">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    <CheckCircle size={16} className="text-green-600" />
                    <p className="text-xs font-medium text-gray-600">Total Pago</p>
                  </div>
                  <p className="text-xl font-bold text-green-700">{formatBRL(totalPago)}</p>
                </div>
              </div>
              ...
              {filtered.length === 0 ? (
                <div className="py-10 text-center">
                  <DollarSign className="mx-auto text-gray-300 mb-2" size={30} />
