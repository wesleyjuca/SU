...
OAUTH_PROVIDERS: dict[str, dict] = {
    ...
    "google_workspace": {
        "authorize_url": "https://accounts.google.com/o/oauth2/v2/auth",
        "token_url": "https://oauth2.googleapis.com/token",
        # Fase pós-262 — achado real (usuário reportou "arquivos gerados não
        # são salvos na pasta configurada", persistente mesmo depois da Fase
        # 258 ter dado um jeito de ESCOLHER a pasta): `drive.file` nunca
        # concede acesso de ESCRITA a uma pasta pré-existente do usuário que
        # foi só LISTADA por uma chamada REST própria (`GET .../folders`,
        # sob `drive.metadata.readonly`) — esse escopo só reconhece arquivos/
        # pastas que o próprio app criou, ou que o usuário abriu
        # explicitamente através do widget oficial "Google Picker" (é o
        # clique no Picker, não o escopo em si, que concede o acesso
        # per-item). Confirmado pela documentação oficial do Google
        # (developers.google.com/workspace/drive/api/guides/api-specific-auth)
        # e por issues oficiais dos clients (google-api-*-client) — o padrão
        # relatado é `parents` rejeitado/ignorado, nunca sucesso. Como este
        # projeto não integra o Google Picker (exigiria SDK JS + API key
        # extra), a correção de causa raiz é o escopo completo `drive`, que
        # a mesma doc oficial confirma dar acesso de escrita a qualquer
        # pasta pré-existente do usuário. `drive.metadata.readonly` sai —
        # `drive` já cobre listagem de sobra.
        #
        # Contrapartida real, não escondida: `drive` é escopo restrito/
        # sensível no Google Cloud Console — pode exigir habilitar o escopo
        # na tela de consentimento OAuth do projeto (ação do dono do
        # projeto, fora do que este código controla) e, se o app crescer
        # além de "modo de teste" (poucos usuários), o processo formal de
        # verificação do Google. Contas já conectadas precisam reconectar
        # (já era o caso desde a Fase 258) — `access_type=offline+
        # prompt=consent` (abaixo) já força a tela de consentimento a cada
        # reconexão.
        "scope": (
