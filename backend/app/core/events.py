...
# DDL idempotente aplicado em TODO boot, depois do `create_all`. É o
# mecanismo de migração real deste projeto (o alembic nunca rodou até a fase
# pós-260.7). Extraído da `lifespan` para uma constante+função porque o passo
# de schema do CI se dizia "mesmo caminho do boot" e na prática pulava este
# bloco inteiro — o banco do CI ficava sem 9 índices que produção tem, 3 deles
# constraints de integridade (`tenants_slug_unique_idx`, `users_email_unique_idx`,
# `uq_process_movements_dedup`), o que fazia `test_tenant_user_unique_constraints`
# pular silenciosamente no CI em vez de proteger.
DDL_IDEMPOTENTE: list[str] = [
...
async def aplicar_ddl_idempotente(engine) -> None:
    """Roda cada DDL em transação própria, fail-soft.

    Uma falha (ex.: índice único num banco com duplicata) vira warning e não
    derruba o boot nem bloqueia os statements seguintes.
    """
    from sqlalchemy import text

    for _sql in DDL_IDEMPOTENTE:
        try:
...
async def lifespan(app: FastAPI):
    ...
    if _has_real_db:
        ...
        await aplicar_ddl_idempotente(engine)
